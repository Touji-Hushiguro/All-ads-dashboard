...
function getPerformance_(projectId, platform) {
  ...
  // 3. 自社CVデータで上書き（全ソースを統合）
  var allCvByAd = getAllCvByAd_();
  if (Object.keys(allCvByAd).length > 0) {
    all = all.map(function (item) {
      var cv = allCvByAd[item.adName];
      if (cv !== undefined) {
        return {
          id: item.id,
...
/**
 * 外部スプレッドシートのCVデータを読む（れいわキャリア用）
 * 顧客データDB: A列=タイムスタンプ, P列(16)=配信媒体, Q列(17)=CR（広告名）
 * 1行 = 1CV。行数をカウントしてCV数とする。
 */
function getExternalCvData_() {
  var url = getConfig_('EXTERNAL_REIWA_CV_URL');
  var name = getConfig_('EXTERNAL_REIWA_CV_NAME');
  if (!url || !name) return {};
  ...
  try {
    ...
    var COL_DATE = 0;      // A列: タイムスタンプ (0-indexed)
    var COL_PLATFORM = 15; // P列: 配信媒体
    var COL_CR = 16;       // Q列: CR（広告名）

    // 最新日を探す
    var maxDate = '';
    rows.forEach(function (row) {
      if (!row[COL_DATE]) return;
      var d = Utilities.formatDate(new Date(row[COL_DATE]), 'Asia/Tokyo', 'yyyy-MM-dd');
      if (d > maxDate) maxDate = d;
    });
    ...
    rows.forEach(function (row) {
      ...
      var d = Utilities.formatDate(new Date(row[COL_DATE]), 'Asia/Tokyo', 'yyyy-MM-dd');
      if (d !== maxDate) return;
      var crName = String(row[COL_CR] || '').trim();
      if (!crName) return;
      cvByAd[crName] = (cvByAd[crName] || 0) + 1;
    });

    return { date: maxDate, cvByAd: cvByAd };
  } catch (err) {
    Logger.log('getExternalCvData_ error: ' + err.message);
    return {};
  }
}

/**
 * 全CVソースを統合（DEN用CV_API + れいわ用外部シート）
 * 広告名をキーにしたCV件数マップを返す
 */
function getAllCvByAd_() {
  var combined = {};

  // 1. DEN用: CV_APIタブ（ローカル）
  var aspCv = getAspCvData_();
  if (aspCv.cvByAd) {
    Object.keys(aspCv.cvByAd).forEach(function (k) {
      combined[k] = (combined[k] || 0) + aspCv.cvByAd[k];
    });
  }

  // 2. れいわ用: 外部スプレッドシート
  var extCv = getExternalCvData_();
  if (extCv.cvByAd) {
    Object.keys(extCv.cvByAd).forEach(function (k) {
      combined[k] = (combined[k] || 0) + extCv.cvByAd[k];
    });
  }
